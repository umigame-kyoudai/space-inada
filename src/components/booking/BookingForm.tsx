...
export function BookingForm({
  ...
  const accessHref = locale
    ? `/${locale}/access#shooting-locations`
    : "/access#shooting-locations";
  ...
  const [referralStaff, setReferralStaff] = useState<ReferralStaff | null>(
    null,
  );
  ...
  const participantCountComplete =
    participantCount > 0 && participantCount <= 10;
  const requiredItems = [
    {
    ...
    },
    {
      ...
      complete: Boolean(preferredTimeWindow),
    },
    { label: t ? t.planLabel : "希望プラン", complete: Boolean(plan) },
    { label: t ? t.nameLabel : "お名前", complete: Boolean(name.trim()) },
    {
      label: t ? t.participantsLabel : "参加人数・性別",
      complete: participantCountComplete,
    },
    { label: t ? t.phoneLabel : "携帯番号", complete: Boolean(phone.trim()) },
    {
      label: t ? t.lateNightConsent : "深夜料金への同意",
      complete: lateNightConsent,
    },
  ];
  const incompleteRequiredItems = requiredItems.filter(
    (item) => !item.complete,
  );
  const completedRequiredCount =
    requiredItems.length - incompleteRequiredItems.length;
  ...
  const subtotal: number | null = useMemo(() => {
    ...
    if (selectedPlan.kind === "perGroup") {
      return (
        (selectedPlan.basePrice ?? 0) + pickupAmount + staffNominationAmount
      );
    }
  ...
  const total: number | null =
    subtotal == null ? null : subtotal - discountAmount;

  // 表示・送信用の合計テキスト
  const totalText = useMemo(() => {
    if (!plan) return "プランを選択すると概算が表示されます";
    if (participantCount === 0) return "人数を入力すると概算が表示されます";
  ...
  }, [plan, total, participantCount, exceedsParticipantLimit, selectedPlan]);

  const totalDisplay = !plan
    ? (t?.totalPlaceholder ?? totalText)
    : participantCount === 0
      ? (t?.participantsEstimate ?? totalText)
      : total == null
        ? (t?.quoteEstimate ?? totalText)
        : formatPrice(total);
  ...
  const breakdown = useMemo(() => {
    ...
    if (selectedPlan?.kind === "perPerson") {
      lines.push(
        t
          ? formatTemplate(t.breakdownAdults, {
              n: adultsNum,
              price: formatPrice(selectedPlan.basePrice ?? 0),
            })
          : `大人 ${adultsNum}名 × ${formatPrice(selectedPlan.basePrice ?? 0)}`,
      );
      if (childrenNum > 0) {
        lines.push(
          t
            ? formatTemplate(t.breakdownChildren, {
                n: childrenNum,
                price: formatPrice(selectedPlan.childPrice ?? 0),
              })
      ...
      lines.push(
        t
          ? formatTemplate(t.breakdownGroup, {
              plan: selectedPlan.label ?? selectedPlan.name,
              price: formatPrice(selectedPlan.basePrice ?? 0),
            })
          : `${selectedPlan.name} 1組 ${formatPrice(selectedPlan.basePrice ?? 0)}`,
      );
    }
    if (pickup)
      lines.push(
        t
          ? formatTemplate(t.breakdownPickup, {
              price: formatPrice(pickupPrice),
            })
          : `送迎 +${formatPrice(pickupPrice)}`,
      );
    if (staffName === "稲田") {
      lines.push(
        t
          ? formatTemplate(t.breakdownNomination, {
              name: "Inada",
              price: formatPrice(staffNominationPrice),
            })
          : `カメラマン指名 +${formatPrice(staffNominationPrice)}（稲田）`,
      );
    } else if (staffName) {
      lines.push(
        t
    ...
    if (coupon && discountAmount > 0) {
      lines.push(
        t
          ? formatTemplate(t.breakdownCoupon, {
              code: coupon.code,
              price: formatPrice(discountAmount),
            })
          : `クーポン（${coupon.code}） -${formatPrice(discountAmount)}`,
      );
    }
    return lines;
  }, [
  ...
  function validateRequiredFields(): boolean {
    ...
    // ネイティブの検証だけでは固定バーの背後へ移動するため、先に欄全体を中央へ表示する。
    ...
    (invalid.closest("label") ?? invalid).scrollIntoView({
      block: "center",
      behavior: "instant",
    });
  ...
  return (
    <div className="booking-form-layout mt-6 grid min-w-0 max-w-full gap-6 lg:grid-cols-2 lg:gap-8">
      {/* 入力フォーム（カード） */}
      <form
        ref={formRef}
        ...
        className="cosmic-panel min-w-0 max-w-full rounded-2xl p-4 sm:p-8"
      >
        <h2 className="text-lg font-bold text-ink">
          {t ? t.formHeading : "予約内容を入力"}
        </h2>
        <div className="mt-3" aria-live="polite">
          <div className="flex items-center justify-between gap-3 text-xs text-muted">
            <p>
              {incompleteRequiredItems.length === 0
                ? (t?.progressComplete ?? "必須項目の入力が完了しました")
                : t
                  ? formatTemplate(t.progressRemaining, {
                      n: incompleteRequiredItems.length,
                    })
                  : `必須項目はあと${incompleteRequiredItems.length}個`}
            </p>
          ...
          <div
            role="progressbar"
            aria-label={t?.progressLabel ?? "必須項目の入力進捗"}
            aria-valuemin={0}
          ...
          >
            <div
              ...
              style={{
                width: `${(completedRequiredCount / requiredItems.length) * 100}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="mt-5 space-y-6">
          ...
          <div>
            ...
            >
              ...
              <option value="相談して決めたい">
                {t ? t.planUndecided : "相談して決めたい"}
              </option>
            </select>
          </div>

          <fieldset className="min-w-0 border-t border-line pt-4">
            <legend className="pr-3 text-sm font-medium text-ink">
              {t ? t.participantsLabel : "参加人数"}
              <RequiredBadge label={requiredText} />
            </legend>
            <p
              id="participant-count-help"
          ...
          <div
            id="booking-estimate"
            className="rounded-lg border border-line bg-mist p-4"
          >
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              <span
                className={`font-bold text-accent ${participantCount > 0 && total != null ? "text-xl" : "text-sm"}`}
              >
                {totalDisplay}
              </span>
            </div>
            {participantCount > 0 && breakdown.length > 0 && (
            ...
            {selectedPlan?.kind === "perGroup" && (
              ...
              >
                {selectedPlan.maxParticipants != null
                  ? t
                    ? formatTemplate(t.groupLimitNote, {
                        max: selectedPlan.maxParticipants,
                        over: selectedPlan.maxParticipants + 1,
                      })
          ...
          <fieldset>
            ...
            <div className="grid gap-2">
              {PREFERRED_TIME_WINDOWS.map((option) => {
                ...
                const translated = t?.timeWindows.find(
                  (w) => w.value === option.value,
                );
                return (
                  <label
                    key={option.value}
                    className={`flex min-h-16 cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 transition-all focus-within:ring-2 focus-within:ring-accent ${
                      selected
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      <span
                        className={`block text-sm font-bold ${selected ? "text-accent" : "text-ink"}`}
                      >
                        {translated?.label ?? option.label}
                      </span>
                      <span
                        className={`mt-1 block text-xs leading-relaxed ${
                          option.value === "24:00〜翌4:00"
                            ? "text-accent"
                            : "text-muted"
                        }`}
                      >
            ...
            <details className="booking-disclosure mt-2">
              <summary>
                {t?.shootingDetailsTitle ?? "撮影時間・集合場所について"}
                <span aria-hidden="true">＋</span>
              </summary>
              <div
          ...
          <details className="booking-disclosure rounded-xl border border-line p-4">
            <summary>
              <span>
                <span className="block text-sm font-semibold text-ink">
                  {t?.optionalDetailsTitle ?? "オプション・その他の情報"}
                </span>
                <span className="mt-1 block text-xs font-normal text-muted">
                  {optionalFieldCount > 0
                    ? t
                      ? formatTemplate(t.optionalDetailsFilled, {
                          n: optionalFieldCount,
                        })
                      : `${optionalFieldCount}項目を入力済み`
                    : (t?.optionalDetailsHint ??
                      "送迎・宿泊先・Instagram・クーポン（任意）")}
                </span>
              </span>
              <span aria-hidden="true">＋</span>
            </summary>
            <div className="mt-4 space-y-5">
              ...
              <fieldset>
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-line bg-white px-4 py-3">
                    ...
                    >
                      <option value="">
                        {t ? t.nominationNone : "指名なし（おまかせ） ¥0"}
                      </option>
                      <option value="稲田">
                        {inadaNominationOption ??
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    t ? t.instagramPlaceholder : "instagram_id（@は不要）"
                  }
                  className={inputClass}
                />
                ...
                >
                  {coupon && discountAmount > 0 && (
                    <p className="font-medium text-emerald-700">
                      {t ? t.couponApplied : "適用"}：{coupon.code} −
                  ...
                  {couponInvalid && (
                    <p className="text-rose-700">
                      {t ? t.couponInvalid : "クーポンが見つかりません"}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="mt-6 flex items-center justify-between gap-3 border-t border-line pt-5">
          ...
          <button
            type="button"
            onClick={handleClear}
            className="min-h-11 shrink-0 rounded-lg border border-line bg-mist px-3 py-2 text-xs text-ink-soft transition-colors hover:border-line hover:text-accent"
          >
      ...
      <div className="min-w-0 max-w-full lg:sticky lg:top-20 lg:self-start">
        <div className="cosmic-panel min-w-0 max-w-full rounded-2xl p-4 sm:p-8">
          <h2 className="text-lg font-bold text-ink">
            {t ? t.previewHeading : "送信内容プレビュー"}
          </h2>
          <p className="mt-1 text-xs text-muted">
            {t
              ? t.previewSubtext
        ...
        <div className="booking-sticky-actions fixed inset-x-0 bottom-0 z-40 border-t border-line bg-white px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2.5 lg:hidden">
          ...
          {incompleteRequiredItems.length > 0 ? (
            <button
              ...
              className="mt-2 flex min-h-12 w-full items-center justify-center rounded-lg bg-accent px-4 text-sm font-semibold text-on-accent hover:bg-accent-hover"
            >
              {t?.continueButton ?? "次の必須項目を入力する"}
